...
export class BorrowAmountModalComponent {
      ...
      async onSubmit() {
        ...
        try {
          ...
          if (this.editMode && this.userData?.id) {
            ...
            const userDocRef = doc(this.firestore, 'borrow', this.userData.id);
            ...
            if (!navigator.onLine) {
              ...
              this.toastr.success('Borrow amount saved successfully');
            }
          } else {
  
            addDoc(collection(this.firestore, 'borrow'), {
              ...payload,
              createdAt: new Date(),
            });
            if (!navigator.onLine) {
