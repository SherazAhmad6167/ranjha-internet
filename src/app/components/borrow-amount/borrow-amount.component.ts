...
export class BorrowAmountComponent {
   isLoading = false;
    isDeleting = false;
    searchTerm = '';
    users: any[] = [];
  ...
  totalProfit: number = 0;
    ...
    async loadExpenses() {
      ...
      try {
        const usersRef = collection(this.firestore, 'borrow');
    ...
    openExpenseModal(userData?: any) {
      const modalRef = this.modalService.open(BorrowAmountModalComponent, {
        size: 'xl',
        backdrop: 'static',
      });
    ...
    async confirmDelete(modal: any) {
      ...
      const userRef = doc(this.firestore, 'borrow', this.selectedDeleteId);
  
      try {
        const userSnap = await getDoc(userRef);
  
        if (!userSnap.exists()) {
          this.toastr.error('Borrow not found');
          return;
        }
  ...
  calculateTotals(data: any[]) {
    ...
    this.totalRecieved = data.reduce((sum, item) => sum + (item.recieved_amount || 0), 0);
  
    this.totalRemaining = data.reduce((sum, item) => sum + (item.amount - item.recieved_amount || 0), 0);
  }
